	...
	*/
	...
	if (logged_in_user) {
		...
		if (logged_in_user_email) {
			$('#user').html ("<img src=\"https://grassroots.tools/grassroots/images/aiss/user\" width=24 height=24 alt=\"The logged-in user\" /> " + logged_in_user_email);			  
		}
...
function format_grassroots_search_result(this_result) {
	...
	if (json['@type'] == 'Grassroots:Study') {
		...
		let url = root_dir + 'fieldtrial/study/' + study_id;
		display_hit_title (grassroots_search_html, null, url, study_name, img, json['provider']);
...
//		grassroots_search_html.push(img_html + 'Grassroots Field Trial: <a style="color:#18bc9c ! important;" class="newstyle_link" href="' + root_dir + 'fieldtrial/' + ft_id + '" target="_blank" >' + ft_name + '</a>');
		...
		let url = root_dir + 'fieldtrial/' + ft_id;
		display_hit_title (grassroots_search_html, null, url, ft_name, img, json['provider']);
...
//		grassroots_search_html.push('<b>' + service + '</b><br/>');
		...
		let url = root_dir + 'service/payload/' + payload_uri;
		display_hit_title (grassroots_search_html, null, url, '<b>' + service + '</b>', img, json['provider']);
		...
		display_hit_title (grassroots_search_html, null, url, title, img, json['provider']);
		...
		display_hit_title (grassroots_search_html, null, null, title, img, json['provider']);

		grassroots_search_html.push(json['id']);

	} else if (json['@type'] === 'Grassroots:Programme') {
		let pi = json['principal_investigator'];
		let program_link = null;

		if (json['so:url'] !== undefined && json['so:url'] !== null) {
			program_link = json['so:url'];
		} 
			
		display_hit_title (grassroots_search_html, null, program_link, title, img, json['provider']);
		...
		display_hit_title (grassroots_search_html, null, publication_link, title, img, json['provider']);
		...
		if (json['so:url'] !== undefined && json['so:url'] !== null) {
			marti_link = json['so:url'];	
		}

		display_hit_title (grassroots_search_html, null, marti_link, title, img, json['provider']);
...
function display_hit_title (grassroots_search_html, prefix, link_url, link_title, link_img, provider) {
	grassroots_search_html.push ('<div class="row">\n');
	
	if (provider) {
		/* 1st column, show the title and link */
		grassroots_search_html.push ('<div class="large_column">\n');
	}

	grassroots_search_html.push ('<a class="newstyle_link hit" href="' + link_url + '">');
	
	if (link_img) {
		grassroots_search_html.push ('<img src="' + link_img + '" />');
	}

	grassroots_search_html.push (link_title);

	grassroots_search_html.push ('</a>');

	if (provider) {
		grassroots_search_html.push ('</div>\n');

		/* 2nd column, show the provider */
		grassroots_search_html.push ('<div class="small_column right">\n');

		let icon = provider['so:logo'];

		console.log (">>>> provider: " + JSON.stringify(provider));

		
		if (icon) {				
			grassroots_search_html.push ('<img src="' + icon + '" ');

			if (provider ['so:description']) {
				grassroots_search_html.push ('alt="' + provider ['so:description'] + '" ');							
			}						

			grassroots_search_html.push ('/>');
		}
		
		/* make the provider title clickable */
		if (provider ['so:url']) {
			grassroots_search_html.push ('<a target="_blank" href="' + provider ['so:url'] + '">');		
		}
		
		if (provider ['so:name']) { 
			grassroots_search_html.push (provider ['so:name']);
		}
